...
import { IOrderMessage, IRatingTypes, IReviewMessageDetails, ISellerDocument } from '@quysterben/jobber-shared';
...
const getSellerById = async (sellerId: string): Promise<ISellerDocument | null> => {
  const seller: ISellerDocument | null = (await SellerModel.findById({
...
const getRandomSellers = async (size: number): Promise<ISellerDocument[]> => {
  const sellers: ISellerDocument[] = (await SellerModel.aggregate([{ $sample: { size } }]).exec()) as ISellerDocument[];
  return sellers;
};
