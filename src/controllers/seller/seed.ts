...
import { BadRequestError, IBuyerDocument, IEducation, IExperience, ISellerDocument } from '@quysterben/jobber-shared';
...
const seed = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    res.status(StatusCodes.CREATED).json({ message: 'Sellers seeded successfully.' });
  } catch (error) {
    next(error);
  }
};
